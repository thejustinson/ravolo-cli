...
const setupProject = async (framework, projectName) => {
    const spinner = ora("Setting up your project...").start();
    const template = framework === "ReactTS" ? "react-ts" : "react";
    try {
        await execPromise(`npm create vite@latest ${projectName} -- --template ${template} --force`);
...
const installPackage = async (projectName, pkg) => {
...
const initNodeProject = async (projectName) => {
    ...
    try {
        ...
        await execPromise(`${projectName !== "." ? `cd ${projectName} ` : ""} && npm init -y`);
...
const installTypeScript = async (projectName) => {
    const spinner = ora("Installing TypeScript...").start();
    try {
        await execPromise(`${projectName !== "." ? `cd ${projectName} ` : ""} && npm install typescript @types/node`);
...
const initTypeScript = async (projectName) => {
    const spinner = ora("Initializing TypeScript...").start();
    try {
        await execPromise(`${projectName !== "." ? `cd ${projectName} ` : ""} && npx tsc --init`);
...
const addNodeFiles = async (projectName, frameworkChoice) => {
    const srcDir = path.join(projectName, "src");
    const indexFile = path.join(srcDir, frameworkChoice == "NodeTS" ? "index.ts" : "index.js");
    const exampleFile = path.join(srcDir, frameworkChoice == "NodeTS" ? "example.ts" : "example.js");
    await fs.mkdir(srcDir, { recursive: true });
    await fs.writeFile(indexFile, `import { exampleFunction } from './example';

console.log('Hello, Node.js with TypeScript!');
exampleFunction();`);
    await fs.writeFile(exampleFile, `export const exampleFunction = () => {
console.log('This is an example function in TypeScript.');
};`);
};
const copyTemplateFiles = async (frameworkChoice, templateDir, targetDir) => {
    ...
    if (frameworkChoice === "ReactJS") {
        await copyFile(path.join(templateDir, "App.jsx"), path.join(targetDir, "src", "App.jsx"));
    }
    else if (frameworkChoice === "ReactTS") {
        await copyFile(path.join(templateDir, "App.tsx"), path.join(targetDir, "src", "App.tsx"));
    }
    await copyDirectory(templateDir, targetDir);
};
const generateWallet = async (projectName) => {
        ...
        choices: [
            {
                name: "Yes",
                value: "yes",
                description: "A new wallet secret key would be generated and saved in wallet.json",
            },
            {
                name: "No",
                value: "no",
                description: "When needed you will be asked to specify the path to the wallet JSON file relative to this folder",
            },
        ],
    });
    if (walletChoice === "yes") {
        ...
        try {
            ...
            await fs.writeFile(path.join(projectName, "wallet.json"), JSON.stringify(key, null, 2));
...
const initializeProject = async () => {
    ...
    if (frameworkChoice === "manual") {
        log.success("You have selected the manual option. Please proceed with the manual setup of your framework/library. The initialization process has been halted.");
        await generateWallet(projectName);
        return;
    }
    const isDefaultProjectName = projectName === ".";
    const dirPath = path.resolve(".");
    if (["ReactJS", "ReactTS"].includes(frameworkChoice) &&
        isDefaultProjectName) {
        const files = await fs.readdir(dirPath, { withFileTypes: true });
        if (files.length > 0) {
            const handleFilesChoice = await select({
                message: "The root directory is not empty. How would you like to proceed?",
                choices: [
